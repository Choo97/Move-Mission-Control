...
class AdminReservationApiControllerTest {
    ...
    void 관리자_예약목록_API는_예약목록을_JSON으로_조회한다() throws Exception {
        ...
        mockMvc.perform(get("/api/admin/reservations")
                ...
                .andExpect(jsonPath("$.content[0].customerName").value("R**"))
                .andExpect(jsonPath("$.content[0].phone").value("010-****-2222"))
    ...
    void 관리자_예약목록_API는_처리필요_예약만_조회한다() throws Exception {
        ...
        mockMvc.perform(get("/api/admin/reservations")
                ...
                .andExpect(jsonPath("$.content[0].customerName").value("처**"))
                .andExpect(jsonPath("$.content[0].attentionRequired").value(true))
                .andExpect(jsonPath("$.totalElements").value(1));
    }
    ...
    void 관리자_알림이력_API는_필터와_검색어를_적용한다() throws Exception {
        ...
        mockMvc.perform(get("/api/admin/notifications")
                ...
                .andExpect(jsonPath("$[0].customerName").value("알**"))
                .andExpect(jsonPath("$[0].phone").value("010-****-1111"))
                .andExpect(jsonPath("$[0].email").value("a***@example.com"))
                .andExpect(jsonPath("$[0].channel").value("EMAIL"))
                .andExpect(jsonPath("$[0].status").value("FAILED"))
                .andExpect(jsonPath("$[0].recipientContact").value("a***@example.com"))
