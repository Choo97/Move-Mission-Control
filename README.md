...
- 관리자 예약 목록
- 관리자 이사 일정 달력
- 관리자 이사 일정 달력 상태별 색상 표시
- 관리자 이사 일정 달력 오늘 일정 강조
- 관리자 이사 일정 달력 작업 버튼
- 관리자 이사 일정 달력 상태 빠른 변경
- 관리자 대시보드 요약 지표
- 관리자 오늘 해야 할 일 대시보드
- 관리자 예약 처리 우선순위 정렬
- 관리자 예약 우선순위 배지 표시
...
- 관리자 예약 상태 변경 확인 메시지
- 관리자 예약 목록 상태 변경 결과 안내
- 관리자 예약 목록 견적 저장 결과 안내
- 관리자 예약 목록 거리 저장 결과 안내
- 관리자 예약 목록 페이징
- 관리자 예약 목록 표시 건수 선택
- 관리자 예약 목록 CSV 다운로드
- 관리자 예약 목록 CSV 다운로드 감사 로그
- 관리자 예약 상세 목록 복귀 상태 유지
- 예약 상태 변경
- 예약 상태 흐름 관리
- 견적 금액 입력
- 관리자 예약 메모
...
관리자 예약 관리에는 `이사 일정 달력` 화면을 추가했습니다. 왼쪽에는 월간 달력으로 예약 분포와 날짜별 건수를 보여주고, 오른쪽에는 선택한 날짜의 예약을 시간순으로 보여줍니다. 혼자 운영할 때는 전체 목록보다 오늘 또는 특정 날짜의 실제 일정을 빠르게 보는 것이 중요하기 때문입니다.

이사 일정 달력에는 예약 상태별 색상을 표시합니다. 접수, 상담중, 견적안내, 확정, 완료, 취소를 색상으로 구분해 날짜별 예약이 어느 단계인지 빠르게 파악할 수 있도록 했습니다.

이사 일정 달력 상단에는 오늘 일정 요약을 배치했습니다. 다른 달을 확인하고 있더라도 오늘 처리할 이사 예약은 계속 확인할 수 있어야 혼자 운영할 때 놓치는 일이 줄어들기 때문입니다.

선택 날짜 일정 카드에는 상세, 날짜 목록, CSV 버튼을 배치했습니다. 달력에서 일정을 확인한 뒤 바로 상세 처리, 해당 날짜 전체 목록 확인, 외부 공유용 CSV 다운로드로 이어질 수 있도록 하기 위해서입니다.

선택 날짜 일정 카드에서는 예약 상태를 바로 변경할 수 있습니다. 상태 변경은 고객 안내와 운영 이력에 영향을 주므로 기존 확인 메시지와 감사 로그 흐름을 그대로 사용하고, 저장 후에는 다시 같은 달력 날짜로 돌아오도록 했습니다.
...
목록에서 상태를 변경하면 다시 목록으로 돌아오고 성공 또는 실패 메시지를 보여줍니다. 목록에서 여러 예약을 빠르게 처리할 때는 상세 화면으로 이동하지 않고 같은 목록 흐름을 유지하는 편이 더 효율적이기 때문입니다.

목록에서 견적을 저장해도 같은 목록으로 돌아오고 저장 결과 메시지를 보여줍니다. 견적 입력 역시 여러 예약을 연속으로 처리하는 작업이라, 상세 화면으로 이동하지 않도록 했습니다.

목록에서 이동 거리를 저장해도 같은 목록으로 돌아오고 저장 결과 메시지를 보여줍니다. 거리 확인 필요 필터를 켠 상태에서 여러 예약의 거리를 빠르게 입력할 수 있도록 하기 위해서입니다.

예약 목록은 10건 단위로 나누어 보여줍니다. 예약이 많아질수록 한 화면에 모든 데이터를 보여주면 화면이 길어지고 처리할 예약을 찾기 어려워지기 때문에, 페이지 단위로 나누어 운영자가 현재 보고 있는 범위에 집중할 수 있도록 했습니다.

예약 목록의 표시 건수는 10개, 20개, 50개 중에서 선택할 수 있습니다. 평소에는 10개 단위로 집중해서 보고, 예약이 많은 날에는 더 많은 행을 한 번에 확인할 수 있도록 검색 필터 영역에 함께 배치했습니다.

예약 목록은 현재 필터와 정렬 조건을 유지한 채 CSV로 다운로드할 수 있습니다. 운영자가 예약 데이터를 엑셀에서 정리하거나 외부에 공유할 때 필요하므로 검색 필터 영역에 함께 배치했습니다.

예약 목록 CSV를 다운로드하면 감사 로그에 다운로드 관리자, 필터 조건, 다운로드 건수를 기록합니다. CSV에는 고객 연락처와 주소가 포함되므로, 개인정보가 외부 파일로 나가는 작업은 추적 가능해야 하기 때문입니다.

목록에서 예약 상세로 들어간 뒤 `목록으로`를 누르면 기존 필터, 정렬, 페이지 위치로 돌아갑니다. 상세 화면에서 상태, 견적, 거리, 메모, 이메일 작업을 처리한 뒤에도 같은 복귀 주소를 유지해 관리자가 작업 흐름을 잃지 않도록 했습니다.
