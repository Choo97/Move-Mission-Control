...
현재 테스트는 고객 예약 신청, 예약 번호와 연락처 기반 조회, 잘못된 연락처 조회 실패, 예약 수정 및 취소 이력, 관리자 예약 상태 변경, 상태 변경 이력 저장, 잘못된 상태 변경 실패, 견적 계산 규칙, 이메일 알림 이력 생성, 쿠폰 할인 적용, 관리자 로그인 및 권한 접근을 검증합니다.
